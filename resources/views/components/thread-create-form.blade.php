    ...
    <div class="relative flex flex-col w-full h-full text-white bg-gray-900 shadow-md rounded-xl bg-clip-border">
        <div class="relative mx-4 mt-4 overflow-hidden text-white bg-gray-900 rounded-none bg-clip-border">
            <div class="flex flex-col items-center justify-center gap-8 mb-8">
                <div>
                    <h5 class="font-sans text-xl antialiased font-semibold leading-snug tracking-normal text-gray-100">
                        {{ $thread ? __('Edit your thread') : __('Create a new thread') }}
                    </h5>
                </div>
                <form class="grid grid-cols-3 gap-3 h-full w-full" hx-post="{{ route('thread.store') }}"
                    hx-target="#messages">
                    @csrf
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="tags" name="tags" type="text" class="mt-1 block w-full" required />
                    </div>
                    <div class="mb-4 col-span-3">
                        ...
                        <div class="messages" id="messages">
                            {{-- @include('components.message', ['message' => 'Successfuly created thread', 'type' => 'success']) --}}
                        </div>
